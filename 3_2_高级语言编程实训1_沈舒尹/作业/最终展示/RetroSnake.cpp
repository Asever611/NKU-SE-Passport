...
// 控制台输出一次玩法指南
void printGuideOnce() {
    wcout << L"============= Retro Snake 玩法指南 =============" << endl;
    wcout << L"1) 单人模式：W/A/S/D 或 方向键 控制移动" << endl;
    wcout << L"2) 双人模式：W/A/S/D 控制玩家1，方向键控制玩家2" << endl;
    wcout << L"3) 数字键 1-9/0 可直接切换难度（0 代表难度10）" << endl;
    wcout << L"4) 按 F 切换单人/双人，空格开始/重开，Q 退出" << endl;
    wcout << L"5) 可点击界面按钮完成同样操作" << endl;
    wcout << L"6) 难度规则：" << endl;
    wcout << L"   2：开始有加速度，难度越高加速度越大" << endl;
    wcout << L"   5：开始有墙，难度越高墙越多" << endl;
    wcout << L"   8：初速提高，难度越高初速越高" << endl;
    wcout << L"================================================" << endl;
}
...
// 颜色淡化
COLORREF lightenColor(COLORREF c, int percent) {
    int r = GetRValue(c);
    int g = GetGValue(c);
    int b = GetBValue(c);
...
// 构建开始菜单按钮
void buildStartMenuButtons(vector<UiButton>& buttons) {
    buttons.clear();

    int boxW = 760; // 居中显示的菜单框宽度
    int boxH = 360;
    ...
    UiButton diffBtn;
    diffBtn.rc = { c1l + 45, cardY + 63, c1l + cardW - 45, cardY + 99 };
    diffBtn.text = L"修改难度(1-9)";
    diffBtn.action = BTN_DIFF_INC;
    buttons.push_back(diffBtn);

    UiButton modeBtn;
    modeBtn.rc = { c2l + 45, cardY + 63, c2l + cardW - 45, cardY + 99 };
    modeBtn.text = L"切换模式(F)";
    modeBtn.action = BTN_TOGGLE_MODE;
    buttons.push_back(modeBtn);

    UiButton startBtn;
    startBtn.rc = { c3l + 45, cardY + 60, c3l + cardW - 45, cardY + 96 };
    startBtn.text = L"开始游戏(space)";
    startBtn.action = BTN_START_GAME;
    buttons.push_back(startBtn);

    UiButton quitBtn;
    quitBtn.rc = { c3l + 45, cardY + 108, c3l + cardW - 45, cardY + 144 };
    quitBtn.text = L"退出(Q)";
    quitBtn.action = BTN_QUIT_GAME;
    buttons.push_back(quitBtn);
}

// 构建结束页右侧按钮
void buildGameOverButtons(vector<UiButton>& buttons, int rightPaneL, int rightPaneR, int paneTop, int paneBottom) {
    buttons.clear();

    int innerL = rightPaneL + 100;
    int innerR = rightPaneR - 100;
    int totalH = paneBottom - paneTop;
    int buttonH = 35;
    int gap = 8;
    int blockH = buttonH * 4 + gap * 3;
    int headerH = 50;
    int availableH = totalH - headerH;
    int startY = paneTop + headerH + max(0, (availableH - blockH) / 2);

    for (int i = 0; i < 4; ++i) {
        int y1 = startY + i * (buttonH + gap);
        int y2 = y1 + buttonH;
        UiButton btn;
        btn.rc = { innerL, y1, innerR, y2 };
        btn.action = BTN_NONE;
        buttons.push_back(btn);
    }

    buttons[0].text = L"切换难度(1-9)";
    buttons[0].action = BTN_DIFF_INC;
    buttons[1].text = L"切换模式(F)";
    buttons[1].action = BTN_TOGGLE_MODE;
    buttons[2].text = L"下一局(space)";
    buttons[2].action = BTN_START_GAME;
    buttons[3].text = L"退出(Q)";
    buttons[3].action = BTN_QUIT_GAME;
}

// 单人历史排名：居中区域内左/右对齐
void drawRankLineCentered(int l, int t, int r, int b, const wstring& label, const wstring& value, int fontSize, COLORREF color) {
    int w = r - l;
    int innerW = (int)(w * 0.6);
    int innerL = l + (w - innerW) / 2;
    int innerR = innerL + innerW;

    settextstyle(fontSize, 0, _T("微软雅黑"));
    settextcolor(color);
    setbkmode(TRANSPARENT);

    RECT rc = { innerL, t, innerR, b };
    drawtext(label.c_str(), &rc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
    drawtext(value.c_str(), &rc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
}

// 双人胜场统计：表格
void drawDualStatsTable(int leftPaneL, int leftPaneR, int paneTop, int paneBottom, int d1, int d2, int t1, int t2) {
    int tableL = leftPaneL + 24;
    int tableR = leftPaneR - 24;
    int tableT = paneTop + 90;
    int tableB = paneBottom - 40;

    int tableW = tableR - tableL;
    int tableH = tableB - tableT;

    int headerH = (int)(tableH * 0.3);
    int rowH = (tableH - headerH) / 2;
    int labelW = (int)(tableW * 0.32);
    int dataW = (tableW - labelW) / 2;

    setlinecolor(lightenColor(RGB(120, 175, 225), 55));
    line(tableL, tableT, tableR, tableT);
    line(tableL, tableB, tableR, tableB);

    int v1 = tableL + labelW;
    int v2 = v1 + dataW;

    int h1 = tableT + headerH;
    int h2 = h1 + rowH;
    line(tableL, h1, tableR, h1);
    line(tableL, h2, tableR, h2);

    drawTextInRect(v1, tableT, v2, h1, L"WASD 玩家", 22, RGB(30, 105, 60));
    drawTextInRect(v2, tableT, tableR, h1, L"方向键玩家", 22, RGB(35, 115, 70));

    drawTextInRect(tableL, h1, v1, h2, L"当前难度", 22, RGB(45, 120, 70));
    drawTextInRect(tableL, h2, v1, tableB, L"总胜局数", 22, RGB(45, 120, 70));

    drawTextInRect(v1, h1, v2, h2, to_wstring(d1), 22, RGB(30, 105, 60));
    drawTextInRect(v2, h1, tableR, h2, to_wstring(d2), 22, RGB(35, 115, 70));
    drawTextInRect(v1, h2, v2, tableB, to_wstring(t1), 22, RGB(30, 105, 60));
    drawTextInRect(v2, h2, tableR, tableB, to_wstring(t2), 22, RGB(35, 115, 70));
}
...
// 绘制按钮（圆角矩形 + 文本）
void drawButton(const UiButton& btn, int fontSize, COLORREF fill, COLORREF border, COLORREF textColor) {
    setfillcolor(fill);
    setlinecolor(lightenColor(border, 55));
    solidroundrect(btn.rc.left, btn.rc.top, btn.rc.right, btn.rc.bottom, 10, 10);
    roundrect(btn.rc.left, btn.rc.top, btn.rc.right, btn.rc.bottom, 10, 10);

    settextstyle(fontSize, 0, _T("微软雅黑"));
    settextcolor(textColor);
    setbkmode(TRANSPARENT);
    drawtext(btn.text.c_str(), (RECT*)&btn.rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
}

// 难度 +1（循环 1~10）
void incDifficulty() {
    g_difficulty = (g_difficulty % 10) + 1;
}
...
void drawStartMenu() {
    ...
    drawTextInRect(c1l, cardY + 10, c1l + cardW, cardY + 70, L"难度设置", 25, RGB(40, 90, 155));
    {
        ...
        buildStartMenuButtons(buttons);
        drawButton(buttons[0], 22, RGB(235, 246, 255), RGB(105, 160, 220), RGB(55, 80, 125));
    }
    ...
    drawTextInRect(c2l, cardY + 10, c2l + cardW, cardY + 70, L"模式设置", 25, RGB(35, 120, 65));
    {
        vector<UiButton> buttons;
        buildStartMenuButtons(buttons);
        drawButton(buttons[1], 22, RGB(235, 255, 245), RGB(120, 195, 145), RGB(45, 105, 65));
    }
    ...
    drawTextInRect(c3l, cardY + 10, c3l + cardW, cardY + 70, L"开始/退出", 25, RGB(150, 95, 30));
    {
        vector<UiButton> buttons;
        buildStartMenuButtons(buttons);
        drawButton(buttons[2], 22, RGB(255, 248, 236), RGB(230, 175, 115), RGB(130, 85, 35));
        drawButton(buttons[3], 22, RGB(255, 248, 236), RGB(230, 175, 115), RGB(120, 80, 30));
    }
...
void drawGameOver() {
    ...
    if (g_selectedMode == MODE_SINGLE) {
        ...
        drawRankLineCentered(leftPaneL + 70, paneTop + 68, leftPaneR - 70, paneTop + 106, L"1st:", first, 28, RGB(35, 80, 140));
        drawRankLineCentered(leftPaneL + 70, paneTop + 108, leftPaneR - 70, paneTop + 146, L"2nd:", second, 28, RGB(35, 80, 140));
        drawRankLineCentered(leftPaneL + 70, paneTop + 148, leftPaneR - 70, paneTop + 186, L"3rd:", third, 28, RGB(35, 80, 140));
        ...
        drawDualStatsTable(leftPaneL, leftPaneR, paneTop, paneBottom, d1, d2, t1, t2);
    }
    ...
    {
        vector<UiButton> buttons;
        buildGameOverButtons(buttons, rightPaneL, rightPaneR, paneTop, paneBottom);
        for (const auto& btn : buttons) {
            drawButton(btn, 22, RGB(255, 248, 236), RGB(230, 175, 115), RGB(132, 88, 34));
        }
    }
}
...
// 处理难度按键（虚拟键码）
void handleDifficultyKeyVk(int vk) {
    if (vk >= '1' && vk <= '9') {
        g_difficulty = vk - '0';
    }
    else if (vk == '0') {
    ...
    while (peekmessage(&msg, EX_KEY | EX_MOUSE)) {
        if (msg.message == WM_KEYDOWN) {
            ...
            if (vk == VK_SPACE)
                ...
                handleDifficultyKeyVk(vk);
        }
        else if (msg.message == WM_LBUTTONDOWN) {
            vector<UiButton> buttons;
            buildStartMenuButtons(buttons);
            for (const auto& btn : buttons) {
                if (pointInRect(msg.x, msg.y, btn.rc)) {
                    if (btn.action == BTN_DIFF_INC) incDifficulty();
                    else if (btn.action == BTN_TOGGLE_MODE) toggleSelectedMode();
                    else if (btn.action == BTN_START_GAME) startNewGame();
                    else if (btn.action == BTN_QUIT_GAME) g_quit = true;
                    break;
                }
            }
        }
...
void handleRunningInput() {
    ...
    while (peekmessage(&msg, EX_KEY | EX_MOUSE)) {
        if (msg.message == WM_KEYDOWN) {
            int vk = msg.vkcode;
            if (vk == 'W') trySetDirection(DIR_UP);
            else if (vk == 'S') trySetDirection(DIR_DOWN);
            else if (vk == 'A') trySetDirection(DIR_LEFT);
            else if (vk == 'D') trySetDirection(DIR_RIGHT);
            else if (vk == 'Q') g_quit = true;
            else if (vk == VK_UP) {
                if (g_roundMode == MODE_DOUBLE) trySetDirection2(DIR_UP);
                else trySetDirection(DIR_UP);
            }
            else if (vk == VK_DOWN) {
                if (g_roundMode == MODE_DOUBLE) trySetDirection2(DIR_DOWN);
                else trySetDirection(DIR_DOWN);
            }
            else if (vk == VK_LEFT) {
                if (g_roundMode == MODE_DOUBLE) trySetDirection2(DIR_LEFT);
                else trySetDirection(DIR_LEFT);
            }
            else if (vk == VK_RIGHT) {
                if (g_roundMode == MODE_DOUBLE) trySetDirection2(DIR_RIGHT);
                else trySetDirection(DIR_RIGHT);
            }
        }
    }
}
