...
public class AuditPlugin extends AbstractOperationServicePlugIn {
    ...
    private static final String KEY_RETURNENTRY_ENTRY = "qqq6_returnentry";
    ...
    private void changeAssetsQty(DynamicObject[] receiptBills) {
        ...
        for (DynamicObject receiptBill : receiptBills) {
            DynamicObjectCollection receiptEntry = receiptBill.getDynamicObjectCollection(KEY_RETURNENTRY_ENTRY);
            ...
            for (DynamicObject row : receiptEntry) {
                ...
                BigDecimal qty = row.getBigDecimal(KEY_QTY);
        ...
        for (DynamicObject asset : assets) {
            BigDecimal newQty = asset.getBigDecimal(KEY_USABLEQTY).add(qtyMap.get(asset.getString("number")));
